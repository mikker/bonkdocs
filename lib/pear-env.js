...
function normalizeDhtConfig(config) {
  if (!config) return { bootstrap: DEFAULT_BOOTSTRAP.slice(), nodes: DEFAULT_NODES.slice() }
  const bootstrap = Array.isArray(config.bootstrap) ? config.bootstrap : DEFAULT_BOOTSTRAP
