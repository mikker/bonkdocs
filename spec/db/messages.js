...
const encoding49 = {
  preencode(state, m) {
    ...
    if (version >= 4 && m.data) c.buffer.preencode(state, m.data)
  },
  encode(state, m) {
    const flags = (version >= 4 && m.data) ? 1 : 0

    c.string.encode(state, m.type)
    c.uint.encode(state, flags)

    if (version >= 4 && m.data) c.buffer.encode(state, m.data)
  },
  decode(state) {
    const r0 = c.string.decode(state)
    ...
    return {
      type: r0,
      data: (version >= 4 && (flags & 1) !== 0) ? c.buffer.decode(state) : null
    }
  }
}
